...
from confitest import login_driver, driver

class TestPersonalAccount:

    # Проверка перехода по клику на «Личный кабинет»
    def test_move_personal_account_from_main_page_move_done(self,driver,login_driver):
    ...
    def test_move_main_page_from_personal_account_by_clicking_constructor_button_move_done(self, driver, login_driver):
        WebDriverWait(driver, 10).until(expected_conditions.visibility_of_element_located(MainPage.personal_account_button))
        driver.find_element(*MainPage.personal_account_button).click()
        WebDriverWait(driver, 10).until(expected_conditions.visibility_of_element_located(PersonalAccountPage.personal_account_form))
    ...
    def test_move_main_page_from_personal_account_by_clicking_main_logo_button_move_done(self, driver, login_driver):
        WebDriverWait(driver, 10).until(expected_conditions.visibility_of_element_located(MainPage.personal_account_button))
        driver.find_element(*MainPage.personal_account_button).click()
        WebDriverWait(driver, 10).until(expected_conditions.visibility_of_element_located(PersonalAccountPage.personal_account_form))
    ...
    def test_logout_personal_account_logout_done(self, driver, login_driver):
        WebDriverWait(driver, 10).until(expected_conditions.visibility_of_element_located(MainPage.personal_account_button))
        driver.find_element(*MainPage.personal_account_button).click()
        WebDriverWait(driver, 10).until(expected_conditions.visibility_of_element_located(PersonalAccountPage.personal_account_form))
